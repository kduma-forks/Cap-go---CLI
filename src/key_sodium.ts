...
import {createSodium} from './api/crypto_sodium';
...
export const createKey = async (options: Options, log = true) => {
  ...
  const { publicKey, privateKey } = createSodium();
  const { publicKey: publicSigningKey, privateKey: privateSigningKey } = createSodium();
  ...
  // check if baseName already exist
  if (existsSync(baseSigningKeyPub) && !options.force) {
    if (log) {
      p.log.error('Public Signing Key already exists, use --force to overwrite');
      program.error('');
    } else {
      return false
    }
  }
  writeFileSync(baseSigningKeyPub, publicSigningKey);
  if (existsSync(baseSigningKey) && !options.force) {
    if (log) {
      p.log.error('Private Signing Key already exists, use --force to overwrite');
      program.error('');
    } else {
      return false
    }
  }
  writeFileSync(baseSigningKey, privateSigningKey);
  ...
  if (extConfig) {
    ...
    extConfig.plugins.CapacitorUpdater.privateKey = privateKey;
    extConfig.plugins.CapacitorUpdater.signingKey = publicSigningKey;
    // console.log('extConfig', extConfig)
    writeConfig(extConfig, config.app.extConfigFilePath)
  }
